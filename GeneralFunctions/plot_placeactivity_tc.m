function plot_placeactivity(data,cellno,significantonly,dFodY)
...
%for c = 1;
    ...
    if significantonly
        ...
        for catrun = 1:length(data.cells{cellno}.categories{c}.dFoY)
            thisdFoY = cat(2,thisdFoY,discretize(...
    ...
    set(gca,'YTick',(0.5-2*length(data.cells{1,3}.categories{1,c}.dFoT):2:0))
    set(gca,'YTickLabel',abs(-length(data.cells{1,3}.categories{1,c}.dFoT):1:1))
    %set(gca,'YTickLabel',flipud(get(gca,'YTickLabel')))
    
    for iter = size(thisdFoT,1):-1:1
            hold on;
            plot(thisft(iter,:),thisdFoT(iter,:)-2*iter,'Color',[.33 .33 .33]);
            if significantonly
                plot(thisft(iter,:),thismask(iter,:)-2*iter,'r');
            end
            plot(thisft(iter,:),thisy(iter,:)-2*iter-0.5);
        end
