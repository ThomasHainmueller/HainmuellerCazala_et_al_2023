function [dataset] = sbx_get_lineartracks3(folder,rois,tforms,signaltype)
...
for n = size(GreenFiles,1):-1:1
    filename = strrep(GreenFiles(n,:),'.sbx','');
    ephysname = strrep(filename,'_nonrigid','.ephys');
     ...
     %try
        ...
        if strcmp(signaltype,'dGoG')
            ...
            thisgreen = sbximport(filename,1,0); % For nonrigid aligned data
