function plot_placeactivity(data,cellno,significantonly,dFodY,signaltype)
...
%for c = 1;
    ...
    thisdFoT = deal(data.cells{cellno}.categories{c}.(signaltype));
    ...
    if significantonly
        thisdFoY = deal(data.cells{cellno}.categories{c}.(signaltype));
        thisdFoY = cat(2,thisdFoY{:});
    else
        thisdFoY=[];
        for catrun = 1:length(data.cells{cellno}.categories{c}.(signaltype))
            thisdFoY = cat(2,thisdFoY,SBdiscretize(...
                data.cells{cellno}.categories{c}.(signaltype){catrun},...
                data.metadata.categories{c}.y{catrun},0.1:0.05:2.1));
        end
    end
    thisdFoY = thisdFoY';
